...
```{r}
#| code-fold: true
model_fit <- Arima(res.fit, order = c(1, 1, 1), seasonal = c(1, 0, 1))
plot(res.fit, col = "blue")
lines(fitted(model_fit), col = "green")
legend(x = "topright", legend = c("res.fit", "fit1"), fill = 4:1)
```
...
```{r}
#| code-fold: true
model_fit <- Arima(res.fit, order = c(1, 1, 1), seasonal = c(0, 1, 1)) 
model_fit %>% forecast %>% autoplot()
```
...
There looks to be some relation between these two variables, as wehen marijuana arrests hit 0, controlled substance arrests rose for the first time.
