...
adf.test(assault_ts)
```
The ADF tests returns a p-value of less than 0.01, meaning we do not have enough evidence to reject the null hypotheses. This series is stationary.
...
```
The ADF test shows that this series is stationary, though we can clearly see that is not true. 
...
```{r}
ts_lags(controlled_sale_ts, lags = c(12, 24, 36, 48))
```
insert text

#### Decomposition
```{r}
plot(decompose(controlled_sale_ts, "additive"))
```
insert text
...
#### ACF and PACF
```{r}
ggAcf(controlled_sale_ts, 100) + ggtitle("ACF Plot for Annual New York City Controled Substance Sale Arrests")
```
insert text

```{r}
ggPacf(controlled_sale_ts, 100) + ggtitle("ACF Plot for Annual New York City Controled Substance Sale Arrests")
...
#### Augmented Dickey-Fuller Test
```{r}
adf.test(controlled_sale_ts)
```
insert text

#### Detrend and Difference
```{r}
detrend <- resid(lm(arrests_by_crime$controlled_substance_sale ~ arrests_by_date$month, na.action = NULL))
difference <- diff(controlled_sale_ts)

grid.arrange(autoplot(controlled_sale_ts),
             autoplot(as.ts(detrend)), 
             autoplot(difference), 
             ncol = 1)
```
...
#### Lag Plots
```{r}
larceny_ts <- ts(arrests_by_crime$larceny, start = c(2006, 1), frequency = 12)

ts_lags(larceny_ts, lags = 1:12)
```
insert text

```{r}
ts_lags(larceny_ts, lags = c(12, 24, 36, 48))
```
insert text

#### Decomposition
```{r}
plot(decompose(larceny_ts, "additive"))
```
insert text

#### ACF and PACF
```{r}
ggAcf(larceny_ts, 100) + ggtitle("ACF Plot for Annual New York City Larceny Arrests")
```
insert text

```{r}
ggPacf(larceny_ts, 100) + ggtitle("ACF Plot for Annual New York City Larceny Arrests")
...
#### Detrend and Difference
```{r}
detrend <- resid(lm(arrests_by_crime$larceny ~ arrests_by_date$month, na.action = NULL))
difference <- diff(larceny_ts)

grid.arrange(autoplot(larceny_ts),
             autoplot(as.ts(detrend)), 
             autoplot(difference), 
             ncol = 1)
```
...
```{r}
ggPacf(marijuana_pos_ts, 100) + ggtitle("ACF Plot for Annual New York City Marijuana Possession Arrests")
...
#### Lag Plots
```{r}
marijuana_sale_ts <- ts(arrests_by_crime$marijuana_sale, start = c(2006, 1), frequency = 12)

ts_lags(marijuana_sale_ts, lags = 1:12)
```
insert text

```{r}
ts_lags(marijuana_sale_ts, lags = c(12, 24, 36, 48))
```
insert text

#### Decomposition
```{r}
plot(decompose(marijuana_sale_ts, "additive"))
```
insert text

#### ACF and PACF
```{r}
ggAcf(marijuana_sale_ts, 100) + ggtitle("ACF Plot for Annual New York City Marijuana Sale Arrests")
```
insert text

```{r}
ggPacf(marijuana_sale_ts, 100) + ggtitle("ACF Plot for Annual New York City Marijuana Sale Arrests")
...
#### Augmented Dickey-Fuller Test
```{r}
adf.test(marijuana_sale_ts)
```
insert text

#### Detrend and Difference
```{r}
detrend <- resid(lm(arrests_by_crime$marijuana_sale ~ arrests_by_date$month, na.action = NULL))
difference <- diff(marijuana_sale_ts)

grid.arrange(autoplot(marijuana_sale_ts),
             autoplot(as.ts(detrend)), 
             autoplot(difference), 
             ncol = 1)
```
...
```{r}
ggPacf(motor_theft_ts, 100) + ggtitle("ACF Plot for Annual New York City Motor Vehicle Theft Arrests")
...
insert text
```{r}
ggPacf(murder_ts, 100) + ggtitle("ACF Plot for Annual New York City Murder Arrests")
...
```{r}
ggPacf(robbery_ts, 100) + ggtitle("ACF Plot for Annual New York City Robbery Arrests")
